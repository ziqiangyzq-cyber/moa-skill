...
name: moa
description: >-
  Mixture-of-Agents decision support. Use when the user prefixes a request with
  "moa" or explicitly asks for a multi-model committee on a hard, high-stakes,
  easy-to-disagree question (architecture/tooling choices, risky business or
  design decisions). Fans the SAME question out to several models from DIFFERENT
  vendors in parallel, drafts a synthesis, runs an adversarial critique round,
  and delivers a decision memo with disagreements and dissent preserved. Not for
  quick lookups or single-answer questions — it deliberately spends N model calls.
---
...
- For each worker with role `propose`, send the Phase-0 input through its adapter using
  ...
  `MOA_TIMEOUT=<timeout_seconds>` for that call. Use `prompts/proposer.md` as the wrapper. Run them in
  parallel (separate Bash calls in one message, or spawn same-family models via your Agent tool).
...
### Draft synthesis (orchestrator = the `aggregator` slot)
- Using `prompts/aggregator.md`, produce a DRAFT synthesis of the N proposals.
...
### Audit
Write a run dir `runs/<timestamp>/` containing: the Phase-0 input actually sent, every raw
R1 and R2 response, the draft, the final, and a one-line `delta` = did R2/​review change the
decision vs the draft? (If final ≈ draft over many runs, R2 is only ratifying — revisit whether
it earns its cost.)
